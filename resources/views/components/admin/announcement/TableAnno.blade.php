...
<table id="example" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                <td>{{$data->id}}</td>
                <td>{{$data->nama_announcement}}</td>
                <td>{{Str::limit($data->deskripsi_announcement, 25)}}</td>
                <td>{{Str::limit($data->isi_announcement, 25) }}</td>
                ...
                <td class="flex text-center items-center justify-center h-full">
                    ...
                    <a href="/admin/announcement/{{$data->id}}" onclick="event.preventDefault(); if (confirm('Are you sure you want to delete this item?')) { document.getElementById('delete-form-{{$data->id}}').submit(); }">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-500">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                        </svg>
                    </a>
                    
                    <form id="delete-form-{{$data->id}}" action="/admin/announcement/{{$data->id}}" method="POST" style="display: none;">
                        @csrf
                        @method('delete')
                    </form>
            ...
            <form action="announcement/{{$data -> id}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-6 overflow-y-auto max-h-[570px]">
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_announcement" value="{{$data->nama_announcement}}" class="p-2 rounded border-2 border-black mb-2 w-96">
                    </div>
                    <div class="mb-4">
                        <p class="font-medium">Deskripsi Announcement</p>
                        <textarea type="text" name="deskripsi_announcement" class="p-2 rounded border-2 border-black w-9/12">{{$data->deskripsi_announcement}}</textarea>
                    </div>
                    <div class="font-medium mb-4">
                        <p class="mb-2">Isi Announcement</p>
                        <textarea id="myeditorinstance" name="isi_announcement" type="text" class="p-2 rounded border-2">{{$data->isi_announcement}}</textarea>
                    </div>
                    <div>
                    ...
                    <div class="mb-3">
                        ...
                        <input
                          ...
                          type="file"
                          id="formFile"
                          name="gambar_announcement" /><span><?php echo $data['gambar_announcement']?>
                      </div>
                      <div class="mb-3">
                        <label
                          for="formFile"
                        ...
                        <input
                          ...
                          type="file"
                          id="formFile"
                          value="{{$data->pdf_announcement}}"
                          name="pdf_announcement" /><span><?php echo $data['pdf_announcement']?>
                      </div>
                </div>
