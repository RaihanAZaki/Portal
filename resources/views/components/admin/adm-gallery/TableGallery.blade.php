...
<table id="example" class="display" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="w-20 h-20"><img src="{{ asset($data->foto) }}" alt=""></td>
                ...
                <td class="">
                    <div class="flex text-center items-center justify-center">
                        ...
                        <a href="/admin/gallery/{{$data->id}}" onclick="event.preventDefault(); if (confirm('Are you sure you want to delete this item?')) { document.getElementById('delete-form-{{$data->id}}').submit(); }">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-500">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                            </svg>
                        </a>
                        <form id="delete-form-{{$data->id}}" action="/admin/gallery/{{$data->id}}" method="POST" style="display: none;">
                            @csrf
                            @method('delete')
                        </form>
                    </div>
...
<div id="extralarge-modal-{{$data->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full h-full overflow-x-hidden overflow-y-auto p-4 bg-gray-100">
    <div class="relative  w-full h-full">
        <div class="relative bg-white rounded-lg h-full w-full shadow">
            <form action="gallery/{{$data -> id}}" method="POST" enctype="multipart/form-data">
                @method('put')
                @csrf
                ...
                <div class="p-6 space-y-6 overflow-y-auto max-h-[570px]">
                    <div class="mb-4">
                        <p class="font-medium">Nama Foto</p>
                        <input type="text" name="nama_foto" value="{{$data->nama_foto}}" class="p-2 rounded border-2 border-black mb-2 w-96">
                    </div>
                    <div class="font-medium mb-4">
                        <p class="mb-2">Tanggal Posting</p>
                        <input type="date" name="tanggal_posting" class="p-2 rounded border-2 border-black w-96" value="{{$data->tanggal_posting}}">
                    </div>
                    <div class="mb-3">
                        <label
                          for="formFile"
